...
export default function SearchOpportunities() {
  ...
  useEffect(() => {
    const fetchHospitals = async () => {
      try {
        ...
        const data = await res.json();
        setHospitals(data);
        setResults(data); // Show all hospitals by default
        // In production, you can uncomment below to filter only verified hospitals
        // setResults(data.filter(h => h.verified));
      } catch (err) {
        console.error("Error fetching hospitals:", err);
  ...
  // Filter whenever filters or hospitals change
  useEffect(() => {
    ...
    if (filters.county)
      filtered = filtered.filter(
        h => h.county?.trim().toLowerCase() === filters.county.trim().toLowerCase()
      );
    ...
    if (filters.accommodation)
      filtered = filtered.filter(
        h => h.accommodation?.trim().toLowerCase() === filters.accommodation.trim().toLowerCase()
      );

    setResults(filtered);

    if (filtered.length === 0 && (filters.county || filters.specialty || filters.accommodation)) {
      toast.error("No opportunities found matching your criteria.");
    }
  }, [filters, hospitals]);

  const expressInterest = (hospital) => {
    toast.success(`Interest expressed for ${hospital.name}. The hospital will be notified.`);
  };
    ...
    <div className="bg-[var(--background)] text-[var(--muted-foreground)] p-8 rounded-2xl shadow-white/20 max-w-7xl mx-auto my-10">
      ...
      <div className="grid md:grid-cols-3 gap-6">
        {results.length === 0 ? (
          ...
          results.map(h => (
            <div key={h._id} className="bg-[var(--accent)] border border-[var(--border)] rounded-2xl p-5 shadow">
              <h3 className="font-bold text-xl mb-2">{h.name}</h3>
              <p><strong>Type:</strong> {h.type}</p>
              <p><strong>Location:</strong> {h.county}</p>
              <p><strong>Beds:</strong> {h.beds || "Not specified"}</p>
